...
J = -(1/m)*sum(sum(Y.*log(a3) + (1-Y).*log(1-a3))) + (lambda/(2*m))*(sum(sum(temp1.^2))+sum(sum(temp2.^2)));
% -------------------------------------------------------------
delta2 = (a3-Y)*(a2)';
delta1  = sigmoidGradient(Theta1*a1);
delta1 = [ones(1,m);delta1];
delta1 = (Theta2)'*(a3-Y).*delta1;
delta1 = delta1*(a1)';
delta1 = delta1(2:end,:);
% =========================================================================
